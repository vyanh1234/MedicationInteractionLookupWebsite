...
public class AuthService {
  ...
  public User register(String username, String password, String name, String role) {
    if (username == null || !username.matches("[A-Za-z0-9_.]{4,40}")) throw new IllegalArgumentException("Tên đăng nhập phải có 4-40 ký tự");
    if (password == null || password.length() < 6) throw new IllegalArgumentException("Mật khẩu cần ít nhất 6 ký tự");
    if (count(username) > 0) throw new IllegalArgumentException("Tên đăng nhập đã tồn tại");
